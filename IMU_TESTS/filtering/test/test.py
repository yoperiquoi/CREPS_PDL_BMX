...
df = pd.read_csv('./test_rc_3.csv', sep=',')
...
for axis in cols_angles:
    df[axis] = df[axis] * pi/180# Transform body frame accelerations into the inertial (Earth) frame
...
def R_z(z):
    ...
    return np.array([[cos(-z),-sin(-z),0],
                     [sin(-z), cos(-z),0],
                     [0,      0,       1]])# Set up arrays to hold acceleration data for transformation
...
line = np.array([df['LINEAR ACCELERATION X (m/s²)'],
                 ...
                 df['LINEAR ACCELERATION Z (m/s²)']])# Set up arrays to hold Euler angles for rotation matrices
...
for i in range(df.shape[0]):
    ...
    earth_linear[:,i] = R_z(yaw[i]) @ R_y(roll[i]) @ R_x(pitch[i]) @ line[:,i]# Create new columns in dataframe for earth frame accelerations    
...
earth_z = np.array([[0,0,1],]*len(x)).T# Initialize body Vectors
body_x = np.empty(earth_x.shape)
body_y = np.empty(earth_y.shape)
body_z = np.empty(earth_z.shape)# Perform inverse frame transformations (body frame <-- earth frame) 
